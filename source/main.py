            ...
            for i in range(4):
                ...
                for _ in range(INTERLEAVE_STEP):
                    ...
                    if buckets[i]:
                        ...
                        others_sorted.append(buckets[i].pop(0))
                        added_any = True
                        if is_white_bucket: current_sni_ru_count += 1
                    else: break
            if not added_any: break
        final = (top_ru + others_sorted)[:MAX_CONFIGS]
        ranked = sorted(final, key=lambda x: x['ping'])
        speed = {r['link']: rank + 1 for rank, r in enumerate(ranked)}
