...
lock = threading.Lock()
api_semaphore = threading.Semaphore(3) # Лимит ip-api (45/мин)
bad_networks = []
ip_cache = {} # КЭШ ДЛЯ API
...
def check_isp_info(ip_str):
    # Проверяем кэш перед тем как мучить API
    if ip_str in ip_cache:
        return ip_cache[ip_str]
    
    with api_semaphore:
        try:
            time.sleep(1.35) # Чуть быстрее, но безопасно
            ...
            if r.get("status") == "success":
                ...
                res = (country, isp_info, is_hosting)
                ip_cache[ip_str] = res
                return res
        except: pass
...
def smart_ping(host, port, sni):
    try:
        ...
        # Уменьшен таймаут до 1.0 - если за секунду не ответил, в топку
...
def main():
    ...
    # ЭТАП 1 & 2: Параллельная загрузка баз и конфигов источников
    print(f"--- 🟢 УСКОРЕННЫЙ ЗАПУСК [{offset}] ---")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as init_executor:
        ...
        bad_networks.extend(f_cf.result())
        bad_networks.extend(f_hz.result())
...
def main():
    ...
    def validate_one_config(config, is_priority, white_sni_only):
        ...
        if white_sni_only != is_white_sni: return

        # Быстрые фильтры строк
        garbage = ["cloudflare", "openproxy", "-udp443"]
        name_l = name.lower()
        if any(x in name_l or x in sni for x in garbage): return
        if not host.replace('.', '').isdigit(): return # Простой чек что это IPv4
        
        # Блокировка лимитов (до тяжелых запросов)
        with lock:
    ...
    def process_step(urls, is_priority, white_sni_only):
        ...
        # Параллельная валидация (25 потоков для компенсации задержек API)
