...
def main():
    ...
    def validate(config, is_priority, is_white):
        ...
        # Определяем лимиты в зависимости от предполагаемой страны и типа
        if is_ru_potential:
            min_p = MIN_RU_PING
        ...
        # Штрафуем подсеть ТОЛЬКО при таймауте или слишком большом пинге
        if not p1 or p1 > max_p:
            with lock:
                failed_subnets[subnet] = failed_subnets.get(subnet, 0) + 1

        # Отбрасываем конфиг, если пинг вне диапазона (включая слишком низкий, но без штрафа за низкий)
        ...
        if not full or full[1] > MAX_JITTER:
            return
        # Проверяем средний пинг тоже по границам
        avg_ping = full[0]
