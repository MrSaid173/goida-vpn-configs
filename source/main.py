...
MAX_CONFIGS = 50 
MAX_TOTAL_SNI_RU, MAX_TOP_RU_SNI = MAX_CONFIGS // 2, 5
MAX_PER_COUNTRY, MAX_PER_SUBNET, MAX_PER_ID, MAX_FAILED_PER_SUBNET = 15, 3, 6, 4
MAX_SAME_SNI_RU, MAX_SAME_SNI_WORLD = 2, 15  
MIN_RU_PING, MAX_RU_PING = 90.0, 400.0
MIN_WORLD_PING, MAX_WORLD_PING = 25.0, 500.0
MAX_RU_PING_XHTTP, MAX_WORLD_PING_XHTTP = MAX_RU_PING + 150, MAX_WORLD_PING + 150

# --- ГЛОБАЛЬНАЯ СТАТИСТИКА ---
STATS = {
    "raw_found": 0, "unique_after_sets": 0, "processed_total": 0,
    "dropped": {
        "technically_broken": 0, "sni_mismatch": 0, "excluded_sni": 0, 
        "sni_usage_limit": 0, "subnet_id_limit": 0, "ping_range_or_timeout": 0, 
        "banned_isp_hosting": 0, "country_mismatch": 0, "jitter_too_high": 0
    },
    "zero_pings": 0, "all_valid_pings": []
}
...
def main():
    ...
    sni_usage_cnt = {} 
    ru_vlm_cnt, ru_vlm2_cnt, x_cnt = 0, 0, 0
    ...
    def validate(config, is_prio, is_white):
        ...
        if is_technically_broken(config):
            with lock: STATS["dropped"]["technically_broken"] += 1
            return
        ...
        with lock:
            ...
            if sni_usage_cnt.get(sni, 0) >= sni_limit:
                STATS["dropped"]["sni_usage_limit"] += 1
                return
            sub = ".".join(host.split(".")[:3])
            if sub_cnt.get(sub, 0) >= MAX_PER_SUBNET or id_cnt.get(cid, 0) >= MAX_PER_ID:
                STATS["dropped"]["subnet_id_limit"] += 1
                return
            if failed_subnets.get(sub, 0) >= MAX_FAILED_PER_SUBNET:
                STATS["dropped"]["subnet_id_limit"] += 1
                return
        ...
        if not p1 or p1 > max_p or p1 < min_p:
            with lock: STATS["dropped"]["ping_range_or_timeout"] += 1
            if not p1 or p1 > max_p:
                with lock: failed_subnets[sub] = failed_subnets.get(sub, 0) + 1
            return
            
        cc, isp, h_stat = check_isp_info(host)
        if not cc or h_stat == "BANNED" or stop_event.is_set():
            with lock: STATS["dropped"]["banned_isp_hosting" if h_stat == "BANNED" else "ping_range_or_timeout"] += 1
            return
        is_ru = (cc == "RU")
        if is_ru != is_ru_pot:
        ...
        if not full or full[1] > MAX_JITTER or not (min_p <= full[0] <= max_p):
            with lock: STATS["dropped"]["jitter_too_high" if full and full[1] > MAX_JITTER else "ping_range_or_timeout"] += 1
            return

        with lock:
            if host in seen_ips: return
            STATS["all_valid_pings"].append(full[0])
    ...
    def fetch_group_data(urls):
        ...
        unique = list(set(raw)); random.shuffle(unique)
        return unique

    raw_extra, raw_std = fetch_group_data(extra_urls), fetch_group_data(std_urls)
    with lock: STATS["unique_after_sets"] = len(raw_extra) + len(raw_std)
    ...
    avg_p = sum(STATS["all_valid_pings"])/len(STATS["all_valid_pings"]) if STATS["all_valid_pings"] else 0
    print(f"\n{'='*50}\n📊 ИТОГОВАЯ СТАТИСТИКА РАБОТЫ:\n{'='*50}")
    print(f"📥 Всего ссылок найдено: {STATS['raw_found']}")
    print(f"🧹 Уникальных ссылок: {STATS['unique_after_sets']}")
    print(f"⚙️ Обработано потоками: {STATS['processed_total']}")
    print(f"\n🚫 ПРИЧИНЫ ОТСЕВА:")
    for reason, count in STATS["dropped"].items():
        print(f"   - {reason.replace('_', ' ').title()}: {count}")
    print(f"\n⚡️ ПИНГ-АНАЛИТИКА:")
    print(f"   - Средний пинг (валид): {avg_p:.1f} ms")
    print(f"   - Ошибочных пингов (0ms): {STATS['zero_pings']}")
    print(f"   - Найдено рабочих: {len(seen_ips)}")
