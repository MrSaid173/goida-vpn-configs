...
# Статистика
total_processed = 0           # всего конфигов обработано
ping_failures = {             # не прошли пинг
    "timeout": 0,
    "high": 0,
    "low": 0,
    "full_fail": 0,           # отвал на full_ping_analysis
}
ping_attempts_success = 0     # сколько раз первый пинг прошёл
ping_attempts_total = 0       # общее попыток full_ping
country_mismatch = 0          # несоответствие имени / ip-api
ipapi_requests = 0
ipapi_failed = 0
total_links = 0               # всего ссылок из источников
stages_passed = []            # этапы (extra white и т.д.)
...
def full_ping_analysis(host, port, sni, initial_ping):
    global ping_attempts_success, ping_attempts_total
    ping_attempts_total += 1
    pings = [initial_ping]
    success_count = 1 if initial_ping else 0
    try:
        for _ in range(3):
...
def check_isp_info(ip_str):
    global ipapi_requests, ipapi_failed, last_api_call
    ipapi_requests += 1
    with lock:
...
def main():
    ...
    def validate(config, is_priority, is_white):
        ...
        with lock:
            ...
            if (sni in sni_domains) != is_white: return
    ...
    print(f"Не прошли пинг:")
    print(f"  - таймаут/None: {ping_failures['timeout']}")
    print(f"  - слишком высокий: {ping_failures['high']}")
    print(f"  - слишком низкий: {ping_failures['low']}")
    print(f"  - отвал на full_ping: {ping_failures['full_fail']}")
    if ping_attempts_success > 0:
    ...
    print(f"Ограничены лимитами:")
    print(f"  - seen_ips / дубли IP: {seen_ips.__len__()}")
    print(f"  - SNI лимит: {sni_usage_counts.get('over_limit', 0)}")  # если добавите счётчик
    print(f"  - subnet/id лимит: {subnet_counts.get('over', 0)}")  # аналогично
    print(f"  - failed_subnets: {sum(failed_subnets.values())}")
    print(f"  - exposed_world лимит: {max(0, exposed_world_count - MAX_EXPOSED_WORLD)}")
    print(f"  - RU/xhttp лимиты: {max(0, ru_vlm_count - MAX_RU_CONFIGS)} RU vlm, {max(0, xhttp_count - MAX_XHTTP)} xhttp")
    print(f"Несоответствие имени / ip-api: {country_mismatch}")
    print(f"Запросы к ip-api: {ipapi_requests} (неудачных: {ipapi_failed})")
    print(f"Этапы пройдены: {', '.join(stages_passed)}")
